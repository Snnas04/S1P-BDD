...
/
CREATE OR REPLACE TYPE Teacher UNDER Member (
        title VARCHAR2(100),
        salary NUMBER(8, 2),
        category VARCHAR2(20),
        CONSTRUCTOR FUNCTION Teacher(
         ...
         title VARCHAR2,
         salary NUMBER
    ) RETURN SELF AS RESULT MEMBER FUNCTION getTeacher RETURN VARCHAR2
);
/
CREATE OR REPLACE TYPE Student UNDER Member (
    enrolid INTEGER,
    birthdate DATE,
    groupId REF Groups,
    gender CHAR(1),
...
DECLARE
    TeacherList_1 TeacherList := TeacherList(
            Teacher('T001', 'Juliette Smith', 'juli', '666123456', 'juliette.smith@pau.cat', 'Bachelor in Computer Science', 2500),
            Teacher('T002', 'Nicole Brown', 'nico', '666111222', 'nicole.brown@pau.cat', 'Data Science Master', 2500)
        );
BEGIN
    NULL;
END;
